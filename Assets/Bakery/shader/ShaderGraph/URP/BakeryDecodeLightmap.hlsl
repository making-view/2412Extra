...
void FullVolumeLighting_float(float3 objPos, float3 worldNormal, float3 viewDir, float ao, out float3 color)
{
    ...
    bool isGlobal = dot(abs(_VolumeInvSize),1) == 0;
    float3 lpUV = (vertexInput.positionWS - (isGlobal ? _GlobalVolumeMin : _VolumeMin)) * (isGlobal ? _GlobalVolumeInvSize : _VolumeInvSize);
    ...
    float3 sh;
    BakeryVolume_float(vertexInput.positionWS, worldNormal, sh);
    color += sh * ao;
